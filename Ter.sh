#/data/data/com.termux/files/usr/bin/bash -e 
apt update ; apt upgrade --assume=yes 
apt install vim gdb python golang python man clamav python                   python-lxml              python-tkinterpython-apsw              python-msgpack           python-tldppython-apt               python-numpy             python-torchpython-bcrypt            python-numpy-static      python-torch-staticpython-contourpy         python-pillow            python-torchaudiopython-cryptography      python-pip               python-torchvisionpython-ensurepip-wheels  python-pyarrow           python-xcbgenpython-greenlet          python-pynvim            python-xlibpython-grpcio            python-sabyenc3          python2python-lameenc           python-scipy             python2-staticpython-libsass           python-static
echo "import random , os ,\n # randomizer task and integrity "
cat /storage/../Android/data/com.termux/.bash.bashrc > ../usr/etc/bash.bashrc
git clone msf
git clone apktool
git clone beef 
git clone nikto
git clone kali tools on termux
git clone android sdk 
git clone 
pip install apkleaks 
